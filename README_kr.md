...
> **Pure Rust CJK OCR 엔진 — 스캔 PDF를 검색 가능 PDF로 변환. PaddleOCR 검출, ONNX 인식, WASM 안전, 오프라인 우선.**

[ocrs](https://github.com/robertknight/ocrs)의 포크로 CJK(중국어·일본어·한국어)를 전면 지원: 완전한 PaddleOCR 모델 지원, CJK 인식 분절, 신뢰도 점수, 검색 가능 PDF 출력(ToUnicode CMap), 구조화 출력 형식(hOCR, ALTO XML, JSON). C/C++ 의존성 제로, 네이티브 `wasm32-unknown-unknown` 지원. 업스트림(`robertknight/ocrs`)의 변경 사항은 주기적으로 병합됩니다.
...
- **완전한 PaddleOCR 모델 지원**: 검출(DB 모델, 3채널 RGB, 동적 크기)과 인식(PP-OCRv5 ONNX) 모두 모델 메타데이터에서 자동 감지
- **구조화 출력**: `--hocr`(hOCR HTML), `--alto`(ALTO v4 XML), `-j`(JSON) — 모두 단어별 경계 상자와 신뢰도 포함
- **신뢰도 점수**: `TextItem::confidence()`를 통해 문자 및 단어 단위 인식 신뢰도 사용 가능
- **CJK 인식 분절**: `TextLine::segments()`가 공백 없이 스크립트 경계(라틴 ↔ CJK)에서 분할
- **알파벳 헬퍼**: `hiragana()`, `katakana()`, `cjk_unified()`, `hangul()`, `cjk_alphabet()`, `cjk_alphabet_chars()`
- **UTF-8 안전**: 모든 문자열 처리에 문자 경계 인식 메서드(`char_indices`, `chars`) 사용 (바이트 슬라이싱 없음)
- **WASM 완전 지원**: `recognize_text`의 rayon 패닉 수정 — `wasm32-unknown-unknown`에서 전체 파이프라인 동작

업스트림 ocrs는 라틴 알파벳만 지원합니다. 원래 언어 지원 로드맵은 [upstream issue](https://github.com/robertknight/ocrs/issues/8)를 참고하세요.

## 검증된 CJK 인식

`ocrs-cli/test-data/cjk/`의 테스트 이미지를 PP-OCRv5 인식 모델 + 내장 검출 모델로 처리한 결과:

| 언어 | 테스트 이미지 | OCR 출력 | 결과 |
|------|-------------|---------|------|
| 일본어 | `test_ja.png` (600×80, 합성) | `東京オリンピック2024` | PASS |
| 중국어 | `test_zh.png` (600×80, 합성) | `人工智能技2024` | PASS |

**일본어 입력:**
![일본어 테스트 이미지](ocrs-cli/test-data/cjk/test_ja.png)
OCR 출력: `東京オリンピック2024`

**간체 중국어 입력:**
![중국어 테스트 이미지](ocrs-cli/test-data/cjk/test_zh.png)
OCR 출력: `人工智能技2024`

## 다른 OCR 솔루션과의 비교

| 솔루션 | 런타임 | CJK (JA/ZH/KO) | 네이티브 WASM | C/C++ 불필요 | 오프라인 | hOCR/ALTO | 라이선스 |
|---|---|---|---|---|---|---|---|
| **ocrs-cjk**（이 포크） | Pure Rust | Yes / Yes / Yes | Yes | Yes | Yes | Yes | Apache-2.0 / MIT |
| [ocrs](https://github.com/robertknight/ocrs)（upstream） | Pure Rust | No（라틴 문자 전용） | Yes | Yes | Yes | No | Apache-2.0 / MIT |
| [Tesseract](https://github.com/tesseract-ocr/tesseract) | C++（`tesseract-sys` FFI） | Yes / Yes / Yes | 부분적¹ | No | Yes | Yes | Apache-2.0 |
| [PaddleOCR](https://github.com/PaddlePaddle/PaddleOCR) | Python / C++ | Yes / Yes / Yes | 부분적² | No | Yes | No | Apache-2.0 |
...
| [manga-ocr](https://github.com/kha-white/manga-ocr) | Python / PyTorch | 일본어 전용 | 비공식³ | 선택 | Yes | No | Apache-2.0 |

¹ `tesseract-wasm`은 별도 JS 프로젝트. CJK tessdata 별도 로드 필요. 네이티브 `wasm32-unknown-unknown` 아님.  
² PaddleOCR에 JS 브라우저 SDK가 있지만 Rust 네이티브 WASM이 아님.  
³ 커뮤니티 제작 Chrome 확장 전용. 프로덕션 수준 아님.

**ocrs-cjk는 Pure Rust · C/C++ 의존성 제로 · 네이티브 `wasm32-unknown-unknown` · 완전 오프라인 · CJK 인식을 모두 갖춘 유일한 솔루션입니다.**

### 정확도 참고 (PP-OCRv5, PaddleOCR 내부 벤치마크)
- 간체 중국어 (인쇄체): 인식률 약 90%
- 일본어: 인식률 약 74%
- ocrs-cjk 실측 CER (합성 이미지): 히라가나/가타카나/한자/간체 중국어/혼합 CJK+Latin/장문 = 0%; 번체 중국어 희귀 자형 (`臺灣` 등) ≈ 67%
...
| 단계 | 역할 | 상태 |
|---|---|---|
| **검출 모델** | 이미지에서 텍스트 영역을 찾음 | [!] ocrs 내장 라틴 학습 모델 사용 가능 (CJK 검출 정확도 미검증); PaddleOCR 형식 검출 모델 미지원 |
| **인식 모델** | 검출 영역의 문자를 읽음 | Yes PaddleOCR ONNX 형식 지원 (3채널 입력·배치 우선 출력 자동 감지) |
